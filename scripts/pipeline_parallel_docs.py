...
def run_evaluation_pipeline(test_cases, output_base_dir="pipeline_parallel_results_docs"):
    ...
    print("--- Phase 1: Pre-processing Original PNGs ---")
    for orig_path in unique_originals:
        orig_basename = os.path.splitext(os.path.basename(orig_path))[0]
        print(f"Processing baseline for: {orig_basename}")
        
        png_size = os.path.getsize(orig_path)
            
        # Create a dedicated directory for the original's diagrams
        orig_diagram_dir = os.path.join(output_base_dir, "images", orig_basename, "PNG")
